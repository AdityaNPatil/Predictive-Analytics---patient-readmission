...
from hdfs import InsecureClient  # Import HDFS client
import warnings
...
# Connect to HDFS
hdfs_client = InsecureClient('http://localhost:9870', user='hadoop')  # Adjust the URL and user as needed
...
def predict_readmission(Gender, Admission_Type, Diagnosis, Num_Lab_Procedures,
    ...
    hdfs_model_path = '/hospital_data/Readmission_Model.pkl'  # Path in HDFS
    
    # Read the model in binary mode
    with hdfs_client.read(hdfs_model_path) as reader:  # No 'binary' argument
        model = pickle.load(reader)  # Load the model directly from the reader
...
st.set_page_config(page_title="Predicting Hospital Readmissions",
                   layout="wide",
                   menu_items={'About': "### This page is created by Big Data Team"})
